    ...
    # this function will generate all possible sequnces of distance d from WildType wt
        for wt in WTs:
            for d in distances:
                ...
                with open('temporary files\\w{0}_d{1}.txt'.format(wt,d),'w')as f:
                    for i in SeqsList:
                        f.write(i+'\n')
                f.close()
...
#==============================================================================        
    ...
    def TrainModel(self,protein,model):
        ...
        #get the data
        with open('DataForCNN_'+protein+'.txt','rb') as fp:
            FullData = pickle.load(fp)
        
        #get the scores    
        ALLseqs = pd.read_csv('allseqs'+protein+'.csv')
        Y = ALLseqs['score']
        
        #reshape the data so the model could load it
        X_train = np.asarray(FullData)
        Y_train = np.array(Y)
        ...
        Model.fit(X_train,Y_train,batch_size = self.ModelParams['batchsize'],
                  epochs=self.ModelParams['epochs'])
...
#==============================================================================  
    def Predict(self,TrainModels=0):
        ...
        Run = pd.read_csv('temporary files\\RunNum.csv')
        Run = Run['Num'].iloc[0]
        Run+=1        
        utiles.createFolder('temporary files\\predictions{0}'.format(Run))
        if TrainModels:
            ...
            MS2model = pickle.load(open('temporary files\\whole library final models\\MS2_model.sav', 'rb'))
            Qbmodel = pickle.load(open('temporary files\\whole library final models\\Qb_model.sav', 'rb'))
            PP7model = pickle.load(open('temporary files\\whole library final models\\PP7_model.sav', 'rb'))
        ...
        for Fidx in range(len(self.Files)):
            ...
            if EOF:
                file1 = open(self.Files[Fidx]+'.txt','r')
                EOF = 0
            ...
            while not EOF:
                ...
                mat = scipy.io.loadmat('structure_matrix_tmp.mat')                #load the matrix
...
#                    tmp = OHTSS+[self.OneHotDic['G']]+[self.OneHotDic['C']]+OH+OHmc
                ...
                DF['seq'] = Seqs
                DF.to_csv('temporary files\\predictions{0}\\predictions{1}.csv'.format(Run,BatchNumber),index=False)
#                if BatchNumber==1:
#                    DF1 = DF
#                else:
