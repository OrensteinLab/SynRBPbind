...
def N_fold_CV(X,Y,folds,model,params={'optimizer':'adam','epochs':1,'loss':'mse',
    ...
    for i in range(len(vec)-1):
        ...
        curr_model.fit(np.asarray(X_train),np.asarray(Y_train),batch_size = params['batch_size'],epochs=params['epochs'],verbose=0)    
...
#==============================================================================
def N_fold_CV1(X,Y,folds,model,params={'optimizer':'adam','epochs':1,'loss':'mse',
                                      'batch_size':16}):
    pos_thresh = 3.5
    neg_thresh = 0
    Rand = np.random.rand(len(Y),1).argsort(axis=0).ravel()
...
#        all_data['fold_{0}'.format(i)]={'Y_test':Y_test,'pred':Predictions}
    ...
    std_AUC = np.std(AUC)
    Mean_Pearson = np.mean(Pears)
    std_pears = np.std(Pears)
    return {'AUC':Mean_AUC,'pearson':Mean_Pearson} ,{'AUC':std_AUC,'pearson':std_pears}    
#==============================================================================
def train_test_model(X_train,Y_train,X_test,Y_test,model,params={'optimizer':'adam','epochs':1,'loss':'mse',
                                      'batch_size':16}):
    pos_thresh = 3.5
    neg_thresh = 0
    # Rand = np.random.rand(len(Y),1).argsort(axis=0).ravel()
